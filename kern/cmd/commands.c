...
struct Command commands[] =
{
		...
		{ "schedPRIRR", "switch & initialize the priority RR scheduler" ,command_schedPRIRR, 3},
		//**************************************//
...
{
	uint8 numOfPriorties = strtol(arguments[1], NULL, 10);
	uint8 quantum = strtol(arguments[2], NULL, 10);
	uint32 starvThresh = strtol(arguments[3], NULL, 10);

	sched_init_PRIRR(numOfPriorties,quantum,starvThresh);
	return 0;
}
