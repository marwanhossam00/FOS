...
//			cprintf("next_env in scheduler id = %d\n", next_env->env_id);
...
void sched_init_PRIRR(uint8 numOfPriorities, uint8 quantum, uint32 starvThresh)
{
	...
	//Comment the following line
	//panic("Not implemented yet");
//	cprintf("Inside sched_init_PRIRR\n");
//	sched_delete_ready_queues();
	init_queue(&ProcessQueues.env_new_queue);
	init_queue(&ProcessQueues.env_exit_queue);
//	cprintf("1\n");
...
//	cprintf("2\n");
//	cprintf("size of quantums = %d\n", sizeof(quantums));
//	cprintf("size of ProcessQueues.env_ready_queues = %d\n", sizeof(ProcessQueues.env_ready_queues)*5);
	quantums[0] = quantum;
	kclock_set_quantum(quantums[0]);
//	cprintf("3\n");
//	cprintf("quantums[0] = %d\n", quantums[0]);
...
struct Env* fos_scheduler_PRIRR()
{
	...
	//panic("Not implemented yet");
//	cprintf("\n---------------------------\n");
//	cprintf("fos_schedueler_PRIRR");
//	cprintf("\n---------------------------\n");
//	cprintf("Inside PRIRR\n");

//	if(e == NULL)	cprintf("environment is NULL\n");
//	else
//	{
////		cprintf("running environment id = %d\n", e->env_id);
//		if(e->env_status == ENV_RUNNING)
//		{
//			sched_insert_ready(e);
//		}
//	}
//	cprintf("\n\n\n Inside fos_sched_PRIRR\n\n\n");
...
//		struct Env * tmp = dequeue(&ProcessQueues.env_ready_queues[i]);
//
//		if(tmp != NULL)
//		{
////			cprintf("env_id = %d\n", e->env_id);
...
//		}
		if (!LIST_EMPTY(&(ProcessQueues.env_ready_queues[i])))
		{
			struct Env * tmp = dequeue(&ProcessQueues.env_ready_queues[i]);
			kclock_set_quantum(quantums[0]);
//			struct Env * lol = find_env_in_queue(&ProcessQueues.env_new_queue, tmp->env_id);
//			remove_from_queue(&ProcessQueues.env_new_queue, tmp);
			return tmp;
		}
	}

	kclock_set_quantum(quantums[0]);
//	cprintf("No Environments in queues\n");
	return NULL;
}
...
//		sched_print_all();
		acquire_spinlock(&ProcessQueues.qlock);
		if(time > starv_Thresh)
		{
//			cprintf("inside starv_Thresh >\n");
...
//				}
				if (!LIST_EMPTY(&(ProcessQueues.env_ready_queues[i])))
				{
					...
					{
						struct Env * e = dequeue(&ProcessQueues.env_ready_queues[i]);
						enqueue(&ProcessQueues.env_ready_queues[i-1],e);
					}
				}
			}
		}
