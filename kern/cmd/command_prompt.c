...
int process_command(int number_of_arguments, char** arguments)
{
	...
	for (int i = 0; i < NUM_OF_COMMANDS; i++)
	{
		if (strcmp(arguments[0], commands[i].name) == 0)
		{
			...
			if((commands[i].num_of_args == (number_of_arguments - 1)) || (commands[i].num_of_args == -1 && (number_of_arguments - 1) >= 1)){
				return i;
			}
			//command is found with invalid number of arguments
			else{
				LIST_INSERT_HEAD(&foundCommands, &commands[i]);
				return CMD_INV_NUM_ARGS;
			}
		}
		//command not found but its chars are subsequence-matched with one or more commands
		else{
			int found = 0, j = 0;
			for(int k = 0; k < strlen(commands[i].name); k++){
				if(commands[i].name[k] == arguments[0][j]){
					++j;
					if(j == strlen(arguments[0])){
						found = 1;
						break;
					}
				}
	...
	if(command_matched)	return CMD_MATCHED;
	//invalid command
	//Set List with zero
	return CMD_INVALID;
}
